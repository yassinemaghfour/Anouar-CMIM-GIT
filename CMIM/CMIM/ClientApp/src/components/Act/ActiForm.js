...
class Actiform extends React.Component {
    state = {
    ...
    handleDeleteClick = () => {
        axios.delete('/api/ActivityPlaces/' + this.state.placdeId)
            .then(res => {
                console.log(res);
                if (res.status == 200) {
                    this.props.history.push("/ActivityPlaces");
                }
            })
    }
    handleCancelClick = () => {
    ...
    handleSubmit = (e) => {
        ...
        if (!this.state.update && this.state.add) {
            axios.post('/api/ActivityPlaces', data)
                .then(res => {
                    if (res.status == 201) {
                        this.setState({
                            open: true,
                            message: "Ajouter avec succès",
                            placdeId: res.data.placdeId
                        });
                    }
                    console.log(res);
                })
            ...
            console.log(this.state.placdeId);

            data.placdeId = this.state.placdeId;
                axios.put(`/api/ActivityPlaces/${this.state.placdeId}`, data)
                .then(res => {
    ...
    componentDidMount() {
        ...
        if (this.props.isAdd == "true")
            return;
        this.setState({
            placdeId: this.props.match.params.mat
        });
        console.log(this.props.match.params.mat);
        axios.get(`/api/ActivityPlaces/${this.props.match.params.mat}`)
            .then(res => {
                console.log('nnnnnnn');
                console.log(res);
                            ...
                            <form className={classes.container} Validate autoComplete="off" style={{ textAlign: "center" }} onSubmit={this.handleSubmit} >
                                ...
                                <Grid container xs={6} >
                                    <Typography variant="h5" gutterBottom style={{ padding: '20px' }} >
                                        {this.state.placdeId != '' ? 'Code de lieu d\'activité : '+this.state.placdeId : 'Nouveau lieu d\'activité'}
                                    </Typography>
                                </Grid>
...
export default withRouter(withStyles(styles)(Actiform));
