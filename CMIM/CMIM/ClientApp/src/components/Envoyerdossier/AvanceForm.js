...
const rows = [
    ...
    { id: 'referance', numeric: false, disablePadding: true, label: 'Réference' },
                ...
                {numSelected > 0 ? (
                    <Typography color="inherit" variant="subtitle1">
                        ...
                        <Typography variant="h6" id="tableTitle">
                             Les dossiers non envoyés à la CMIM
          </Typography>
                    )}
            </div>
            ...
            axios.get('api/dossiers')
                .then(res => {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        if (res.data[i].etat == "Avancé") { dos.push(res.data[i]); }
                    }
                ...
                onClick: () => {
                    ...
                    .then(res => {
                        if (res.status == 200) {
                            ...
                            {
                                // console.log(this.state.data[i].employee.first_name)
                    const pdf = new jsPDF('p', 'pt', 'letter');
                    pdf.text(220, 30, "Liste des dossiers envoyés à la CMIM");
                    var row = [];
                    for (var i = 0; i < this.state.data.length; i++) {
                        var dos = [];
                        dos.push(this.state.data[i].referance);
                        dos.push(this.state.data[i].employee.matriculecmim);
                        dos.push(this.state.data[i].employeematricule);
                        dos.push(this.state.data[i].employee.last_name + " " +  this.state.data[i].employee.first_name);
                      
                        if(this.state.data[i].conjoint != null || this.state.data[i].enfant != null){
                            dos.push(this.state.data[i].conjoint.lastname + " " + this.state.data[i].conjoint.firstname);
                            
                            
                        }else if(this.state.data[i].enfant != null){
                            dos.push(this.state.data[i].enfant.lastname + " " +  this.state.data[i].enfant.firstname);
                        }
                        
                        
                        else if (this.state.data[i].conjoint == null && this.state.data[i].conjoint == null){
                            dos.push(this.state.data[i].conjoint="vide"); 
                          
                        }else if(this.state.data[i].enfant == null){
                            dos.push(this.state.data[i].enfant="vide");
                        }else if(this.state.data[i].conjoint == null){
                            dos.push(this.state.data[i].conjoint="vide");
                        }
                        /*if(this.state.data[i].enfant != null ){
                           
                            dos.push(this.state.data[i].enfant.lastname + " " +  this.state.data[i].enfant.firstname);
                            
                        }*/
                        
                        dos.push(this.state.data[i].montant);
                    
                        row.push(dos);
                    }
                    pdf.autoTable({
                        head: [['Numéro dossiers CMIM', 'Matricule CMIM', 'Matricule employé', 'Nom et prénom employé', 'Nom prénom benificaire', 'Mt Dossier']],
                        body: row
                        ...
                        pdf.save("Liste_Des_Dossiers_Envoyés.pdf");
                });
                            console.log('test');
                        }
    ...
    render() {
        ...
        return (
            <Paper className={classes.root}>
                ...
                <div align="center">
                {/* <Link to={{pathname: '/dossiers'}}> */}
                    <Fab variant="extended" color="primary" style={{margin: "20px"}} aria-label="Add" onClick={() => { this.handlePrintClick('Avancé') }} className={classes.margin}>
                        <NavigationIcon className={classes.extendedIcon} />
                        Envoyer les dossiers à la CMIM
                </Fab>
                {/* </Link> */}
                </div>
...
export default withAuth(withStyles(styles)(Avance));
